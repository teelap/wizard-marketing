...
- **W31 — the floor fell further, and it fell exactly where W30 predicted.** 6 posts (all cross-posted video, no evergreen deal by design, **no reactives published**), **8 measured impressions for the entire week, median 1**. W30 was 46 imp / median 2 across 10 posts. **Reach fell ~82% in the one week the reactive layer went dark** — which is the cleanest possible confirmation of the W30 read that **reactive newsjacks were the only thing distributing on this surface** (13 / 11 / 8 imp then, versus 1 / 1 / 1 / 5 for cross-posted video now). The W31 batch's decision to hand Threads entirely to the reactive routine was correct in principle; the routine then published nothing, so the surface got no content it could distribute. **No tier move — no dealt card published, and single-digit totals are rounding, not signal.** Threads is now the weakest surface by an order of magnitude: 8 impressions against X's 338 and LinkedIn's 457 for comparable effort. The keep-or-drop decision is Jake's and is escalated again (W31 review §6.5). *(W31 review §2.2)*
- **W32 — the "does Threads distribute video" test was designed, dealt, and never ran.** The W32 batch routed **12 spine reels** here specifically to test whether the surface distributes video when something is actually posted to it. **One published.** Totals: 4 posts, **15 imp / 0 int, median 4** (up from 8, but only because four cross-posted video units landed — 7 / 4 / 4, one unreported). **The test is still unrun**, and the blocker is throughput, not the platform. Zero reactives published for the second straight week, so the one mechanic W30 identified as working here has now been dark for 14 days. **No tier move; single-digit reach is rounding, not signal.** Note for the keep-or-drop decision: Threads got a *deliberate* 12-reel investment this week and returned 15 impressions — the cost side of that decision is now concrete. *(W32 review §2.2)*
...
4. **The Confession** — *when you have a real reversal.* Vulnerability opens space for theirs.
   - "i got this wrong for years." · "i used to sell the feature. the feature was never the point." · "took me 13 years to learn the simple version was the right one."
...
7. **The Hard Number** — *when a §2 stat reframes the stakes.* Specificity is the interrupt.
   - "one client. 3 crews to 9. same market, clearer message." · "one button change. roughly 1.2 to 6.5 percent." · "13 years in. the hardest system to debug is still people."
