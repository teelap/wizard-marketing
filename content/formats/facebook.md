...
- **W31 — CORRECTION: the W30 container finding did not reproduce. `type: POST` and `type: REEL` performed identically.** Seven cross-posted reels published, **1,131 imp / 3 int, median 203**. Split by the field W30 blamed: **`facebookData.type: "POST"` → 212 / 78 / 203 imp (median 203)**; **`type: "REEL"` → 201 / 4 / 209 / 224 (median 205)**. No gap. Every one of the seven — including all three sent as `POST` — landed on a `facebook.com/reel/…` URL and reported as `REEL` in analytics, i.e. **Metricool/Meta normalizes an attached video to a Reel regardless of the flag.** W30's "one field cost 20x reach" read was n=1 and wrong; it is retracted. Setting `type: REEL` is still the correct, explicit thing to do — it just is not the reach lever we thought. **The real W30 outlier needs a different explanation, and W31 offers one:** the single flop this week (`what-is-schema` spine reel, **4 imp vs median 203**) was FB's **second post of that same day** and the earlier of the two — the later one that day pulled 209. Candidate read, n=1, low confidence: **FB throttles a same-day second post.** Test before believing it. *(W31 review §2.2, §4)*
- **W31 — engagement is still absent, 2nd week running.** 3 interactions across 1,131 impressions (0.27%). W30's "FB is a reach surface, not a conversation surface" holds and hardens. Keep it cross-post-only; author nothing native here.
- **W32 — the container retraction is confirmed a third time, and the spine took Facebook's top reel.** 4 cross-posted reels, **444 imp / 2 int, median 104.5**. The week's biggest FB post — the spine clip `sell-perceived-value-than-price` at **217 imp = 2.08x median** — was created with **`facebookData.type: "POST"`** and still landed on a `facebook.com/reel/…` URL and reported as `REEL`. **Meta normalizes an attached video to a Reel regardless of the flag; the field is not a reach lever.** Setting `REEL` explicitly remains correct practice (it controls title/placement intent), it just does not move distribution. W30's claim stays retracted. *(W32 review §2.2)*
- **W32 — engagement absent for a 3rd straight week: 2 interactions on 444 impressions (0.45%).** Three weeks, three near-zeroes (1.01% → 0.27% → 0.45%) while reach stays healthy. This is no longer a small-N observation, it is the platform's steady state for this account. **Treat Facebook as a pure reach surface: cross-post only, never author native, never deal a card that depends on comments or a reply-bait close here.** The spine, however, is worth routing here permanently — both of the week's above-median FB reels were back-catalog clips.
